---
name: amz-private-label
description: >-
  Plan a private-label product from idea to launch readiness. Covers product
  selection criteria, differentiation, supplier sourcing and sampling, unit
  economics, branding and packaging, and the pre-launch checklist. Use when a
  user asks about starting a private-label product, launching their own brand,
  going from idea to product, choosing a private-label product, or a
  private-label roadmap. Trigger phrases: "private label", "start a brand",
  "launch my own product", "private label roadmap", "from idea to product".
...
3. **If a gate fails, stop.** Name what must change before money moves to the next
   stage. Do not let a seller skip ahead.
4. **If a gate passes, define the next stage's actions** and its gate.
5. **Flag the cash plan.** Name roughly what each remaining stage costs so the seller
