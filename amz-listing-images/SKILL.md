---
name: amz-listing-images
description: >-
  Plan a complete Amazon product image set that converts. Builds the shot list
  for the main image and the gallery, designs each infographic, plans lifestyle
  scenes, and sets the order so the gallery answers objections in sequence. Use
  when a user asks about listing images, the main image, the photo gallery,
  infographics, lifestyle images, image order, or what photos a listing needs.
  Trigger phrases: "listing images", "main image", "product photos", "image
  gallery", "infographics", "lifestyle images", "shot list". Works with zero
  tools. the user describes the product and its buyers.
metadata:
  author: Jay GPT Pro
...
1. **Collect inputs.** The product, the target buyer, the top 3 buyer objections, the
   main competitor, what is in the box, and any guarantee or credential.

2. **Brief the main image.** Angle, lighting, fill, against the rules. Note one or two
   variants worth A/B testing.

3. **Assign the 7 frames.** Map each gallery frame to its job, dropping or doubling a
   frame only with a reason.

4. **Design each infographic.** For frames 2, 3, and 6, specify the callouts, the
   exact short text, and the icon style. Keep text large enough to read on a phone.

5. **Brief the lifestyle frame.** The real buyer, the real setting, the emotion. not
   a sterile studio shot with a model who does not match the audience.

6. **Sequence to objections.** Confirm the gallery, read in order, answers the 3 named
