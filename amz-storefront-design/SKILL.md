---
name: amz-storefront-design
description: >-
  Plan an Amazon Brand Store. designs the page structure, the navigation, the
  shoppable content, and the traffic plan so the Store works as a real branded
  destination, not a dead page. Use when a user asks about the Amazon Store,
  Brand Store, storefront design, store pages, shoppable images, or driving
  traffic to a store. Trigger phrases: "amazon store", "brand store",
  "storefront", "store design", "store pages", "shoppable images", "store
  traffic". Works with zero tools.
metadata:
  author: Jay GPT Pro
...
1. **Collect inputs.** The brand, the catalog and how it groups (families, use cases,
...
3. **Design the category and collection pages.** One per family or use case. Curate
   each. Decide which pages also serve as Sponsored Brands landing pages.

4. **Plan the shoppable content.** Which lifestyle scenes become shoppable, and which
   dynamic modules (deals, best sellers, new arrivals) keep the Store fresh.

5. **Build the traffic plan.** Sponsored Brands to the Store, the brand byline,
   external traffic pointed at the Store. Name the primary feed.

6. **Set the review cadence.** A Store is not set-and-forget. seasonal refreshes and
